...
export default ({ className, heading, subheading, cta, secondaryCta, background }) => {
  ...
  if (background) {
    ...
    localBackground = 'https://www.fillmurray.com/1920/600';
  }

  if ('data:null' === localBackground) {
    localBackground = 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7';
  }

  return (
    <div className={`hero cover bg-left bg-center-l relative z-1 overflow-hidden ${className || ''}`} style={{backgroundImage: `url(${localBackground})`}}>
      <div className="tc-l mt4 mt5-m mt6-l ph3">
        <div className="hero--inner relative z-1 pv6">
          {heading && (
