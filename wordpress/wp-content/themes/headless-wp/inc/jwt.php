 ...
 * via the headless_wp_asset_token_content action.
 */

// Generate content asset tokens.
function headless_wp_jwt_generate_asset_token( $asset_id, $type, $extra_meta = 0 ) {
	...
	$token = array(
		...
		'iat'  => time(),
		'nbf'  => time() - 10,
		'exp'  => apply_filters( 'headless_wp_asset_token_expiration', time() + 3600 ),
		'data' => array(
			'asset' => array(
				'asset_id'   => $asset_id,
				'type'       => $type,
...
function headless_wp_jwt_decode_asset_token( $token ) {
	Firebase\JWT\JWT::$leeway = 60;
...
add_action(
	...
	function() {
		...
		if ( 0 === stripos( $url_path, 'asset/' ) ) {
			...
			$token = apply_filters( 'headless_wp_asset_token', array(), $token_string );

			if ( ! empty( $token ) ) {
				list( $asset_id, $type, $extra_meta ) = $token;

				// Token is not blocked. Proceed to run asset actions.
				do_action( 'headless_wp_asset_token_content', $asset_id, $type, $token_string, $extra_meta );
			} else {
...
add_filter(
	'headless_wp_asset_token',
	function( $token, $token_string ) {
		if ( $token_string ) {
			$token = headless_wp_jwt_decode_asset_token( $token_string );
			...
			if ( $token ) {
				...
				if ( ! in_array( $token_string, $blocked_tokens, true ) ) {
					...
					if ( apply_filters( 'headless_wp_asset_expiry_headers', true ) ) {
