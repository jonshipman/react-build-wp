...
function headless_wp_media_upload_action() {
	...
	if ( ! empty( $token ) && ! is_wp_error( $token ) ) {
		...
		if ( current_user_can( 'upload_files' ) ) {
			if ( isset( $_FILES['file'] ) ) {
					...
					do_action( 'headless_wp_media_upload', $file, $mime );
					die;
			}
		} else {
...
add_action( 'wp_ajax_nopriv_media_upload', 'headless_wp_media_upload_action' );
add_action( 'wp_ajax_media_upload', 'headless_wp_media_upload_action' );
...
function headless_wp_media_library_upload( $file, $mime ) {
...
add_action( 'headless_wp_media_upload', 'headless_wp_media_library_upload', 10, 2 );
