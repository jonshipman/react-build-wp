 ...
 */
export default withApolloClient(class extends Component {
  state = {
    cards: [],
    pageInfo: {},
  ...
  executeQuery = async () => {
    if (this.props.client) {
      ...
      if (data?.posts?.edges?.length > 0) {
        this.nextState = { cards: data.posts.edges, pageInfo: data.posts.pageInfo };

        if (this.props.isMounted()) {
          this.setState({ triggerAnimation: !this.state.triggerAnimation, error, loading });
        }
      }
    }
  };
